...
// Per-topic manual text keyed by normalized topic. Aliases share one value
// (e.g. cwd/workspace, threads/sessions) by registering the same array.
const TOPIC_GROUPS = [
  {
    topics: ["new"],
    lines: [
...
const TOPIC_MANUALS = new Map();
for (const { topics, lines } of TOPIC_GROUPS) {
  const text = lines.join("\n");
  for (const topic of topics) {
    TOPIC_MANUALS.set(topic, text);
  }
}
...
export function commandManual(topic = "") {
  ...
  const manual = TOPIC_MANUALS.get(t);
  if (manual) {
    return manual;
  }
